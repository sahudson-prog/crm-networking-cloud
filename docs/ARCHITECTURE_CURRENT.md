...
| `cloud/web/components/ContactProfile.tsx` | Ficha cloud de contacto | Bloque madre cloud de contacto: datos/estado, interacciones, referidos y Coach contextual filtrado. Reutiliza `CoachModule`, iconos/botones globales, accion interna para guardar estado networking y abre el editor global de contacto desde el icono editar. Las filas de interaccion reciben participantes desde `interaction_participants` y muestran un indicador informativo cuando una interaccion es compartida; tambien reciben `external_interaction_sources` para mostrar origen externo/link si existe |
...
| `cloud/web/lib/googleInteractionClient.ts` | Cliente read-only Gmail/Calendar | Lee Gmail y Google Calendar con access token autorizado, paginacion acotada, limites por corrida, errores claros de permisos/token y cursor incremental de Calendar. No guarda datos ni escribe en Google |
| `cloud/web/lib/googleInteractionSyncFlow.ts` | Flujo Google de sync de interacciones | Ensambla contactos app, cursores, cliente Google, adaptador Google y orquestador comun para importar Gmail/Calendar como interacciones app vinculadas a origen externo. Guarda cursores solo si el lote queda ok y no es dry-run; si Calendar informa cursor vencido, marca el cursor y reintenta lectura historica |
