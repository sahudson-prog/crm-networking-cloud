...
- 2026-08-11: Se diagnostica que aplicar `Duplicados complejos` podia dejar contactos locales huerfanos si la creacion insertaba primero `contacts` y luego fallaba al insertar un telefono/correo ya existente. `contactSyncApply` ahora valida conflictos de correo/telefono antes de crear el contacto, evitando nuevas fichas vacias por fallas parciales. Pendiente: limpiar datos huerfanos ya creados y convertir creacion/aplicacion de contactos desde sync en una operacion transaccional.
