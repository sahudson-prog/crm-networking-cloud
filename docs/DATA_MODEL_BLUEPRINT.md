...
- Todo dato privado debe pertenecer a un `user_id`; los catalogos transversales de la app no deben llevar `user_id` y deben distinguirse visualmente como globales en herramientas de diagnostico.
...
- Los nombres de tablas, columnas, vistas, funciones y resultados de diagnostico deben describir el dominio y la unidad de negocio que representan. Evitar nombres genericos como `section`, `key`, `value`, `data` o `payload`, salvo contratos JSON deliberados y documentados.
- El modelo de cuentas, roles, planes, organizaciones, conexiones externas, logs y analitica debe validarse contra `docs/PRIVACY_SECURITY_COMPLIANCE.md` antes de abrir beta multiusuario. La privacidad no es solo UI: debe quedar respaldada por RLS, capabilities, auditoria, minimizacion de datos, consentimiento y separacion entre datos privados y agregados.
...
| `headhunter_companies` | Catalogo global de empresas headhunter oficiales | `id`, `display_name`, `normalized_name`, `notes`, `is_active`, `created_at`, `updated_at` |
| `headhunter_company_domains` | Catalogo global de dominios asociados a una empresa headhunter | `id`, `company_id`, `domain`, `normalized_domain`, `is_primary`, `is_active`, `created_at`, `updated_at` |
| `objectives` | Objetivos de busqueda profesional declarados por el usuario | `id`, `user_id`, `objective_name`, `objective_name_normalized`, `objective_type`, `priority_level`, `objective_description`, `is_active`, `created_at`, `updated_at` |
| `contact_objective_assignments` | Relacion many-to-many entre contactos y objetivos | `id`, `user_id`, `contact_id`, `objective_id`, `assigned_by_actor`, `assigned_at`, `created_at`, `updated_at` |
...
| `external_contact_snapshots` | Foto cruda del contacto en una fuente externa | `id`, `user_id`, `connected_account_id`, `provider`, `external_id`, `display_name`, `company`, `role`, `emails`, `phones`, `birthdays`, `metadata`, `content_hash`, `is_deleted`, `last_seen_at` |
...
| `sync_run_logs` | Bitacora operativa de sincronizaciones | `id`, `user_id`, `run_id`, `provider`, `resource_type`, `operation`, `scope_label`, `step`, `status`, `detail`, `metadata`, `created_at` |
...
| `imports` | Cargas CSV/Excel u otras fuentes manuales | `id`, `user_id`, `file_name`, `file_type`, `status`, `rows_total`, `rows_imported` |
| `data_exports` | Respaldos/exportaciones futuras de la app | `id`, `user_id`, `export_type`, `manifest_json`, `file_hash`, `created_at` |
| `import_batches` | Lotes de carga o migracion controlada | `id`, `user_id`, `source_type`, `source_filename`, `manifest_json`, `validation_report_json`, `status`, `imported_at` |
...
### Migraciones y cargas controladas

Objetivo vigente: la app cloud debe operar con fuente de verdad propia y datos cargados desde conectores o archivos compatibles con el modelo nuevo.

Reglas base:

- Contactos, correos y calendarios deben cargarse primero desde conectores de proveedor, partiendo por Google read-only en v1.
- Los datos que no existan en el proveedor, como minutas, referidos, estados y preferencias, se migran despues con una herramienta separada.
- La app no debe tener columnas, fallbacks ni reglas permanentes para interpretar formatos de una app anterior.
- Toda migracion de continuidad debe transformar la informacion al modelo nuevo antes de escribir en Supabase.
- Antes de escribir, la migracion debe mostrar conteos, errores bloqueantes, advertencias y muestras controladas sin exponer datos sensibles por defecto.
- Cada migracion debe quedar registrada como lote (`import_batches`) y, cuando corresponda, en `audit_log`.
...
- Cualquier migracion desde una herramienta anterior debe mostrar preview con conteos antes de insertar o reemplazar datos, y debe vivir fuera del runtime cloud.
...
- Los IDs externos de Google, Microsoft, Apple u otros proveedores deben guardarse en tablas de referencia externa, nunca como ID principal del contacto.
- Las interacciones de la app se identifican por `interactions.id`; los objetos de proveedor viven en `external_interaction_sources`.
- La migracion de continuidad para minutas, referidos y estados debe adaptar datos al modelo nuevo antes de dejarlos disponibles para la app.

Validaciones minimas antes de permitir una migracion de continuidad:
...
- Duplicados de contactos, correos, telefonos, eventos externos, referidos y sugerencias.
...
- Las herramientas historicas usadas durante la primera carga de desarrollo viven en `tools/legacy_migration`.
- No forman parte de la app cloud ni del contrato normal de datos.
...
- La app cloud debe poder operar desde cero con datos cargados por conectores Google.
- Para un usuario con datos existentes, una migracion debe crear un `import_batch`, mostrar diferencias y pedir confirmacion antes de reemplazar, fusionar o ignorar.
- Cada fila migrada debe quedar asociada a `user_id`.
- Cada migracion debe dejar registro en `import_batches` y, cuando corresponda, `audit_log`.

### Schema Postgres v0.1

Archivo de referencia: `cloud/supabase/schema_v0_1.sql`.

Objetivo: crear una base Supabase/Postgres vacia para operar la app cloud con fuente de verdad propia, referencias externas por proveedor y conectores Google de lectura en v1.
...
| Integraciones | `service_connectors`, `connected_accounts`, `external_contact_ids`, `external_contact_snapshots`, `external_interaction_sources`, `external_interaction_read_diagnostics`, `sync_cursors` |
...
| Uso, observabilidad, auditoria y KPIs | `usage_limits`, `usage_events`, `sync_run_logs`, `audit_log`, `metric_snapshots` |

Decisiones del schema v0.1:

- `contacts.id` es el ID propio de la app; los IDs externos deben vivir en tablas de referencias por proveedor, no como columnas legacy dentro de `contacts`.
- Los medios de contacto viven separados en `contact_emails` y `contact_phones`.
- Los objetivos de busqueda profesional son entidades propias del usuario. No se guardan como hashtags libres dentro del contacto. La relacion `contact_objective_assignments` permite asociar un contacto a multiples empresas, industrias, cargos o funciones objetivo, y permite renombrar el objetivo sin romper asociaciones existentes.
- Las metricas de objetivos se calculan inicialmente como datos derivados: objetivo activo + contactos asociados + participantes de interacciones + interacciones tipo `calendar`/`call`. No crear tablas agregadas hasta necesitar historico, performance o auditoria de indicadores. El calculo debe contar cafes unicos por objetivo, poder incluir objetivos sin contactos cuando el usuario lo pide, acotarse a la fecha de inicio de networking en Dashboard y usar el estado networking mas avanzado de sus contactos como estado resumido del objetivo.
- `contact_emails` y `contact_phones` permiten que el mismo correo o telefono exista en mas de un contacto. La unicidad operativa es por `user_id + contact_id + valor normalizado`; la app no bloquea la importacion inicial por coincidencias de segundo orden. La revision de duplicados posterior decide si esos contactos deben fusionarse.
- Contactos conectados separan tres responsabilidades: `contacts` es la ficha local/canonica editable por el usuario; `external_contact_ids` es el puente entre ficha local y proveedor; `external_contact_snapshots` conserva la foto cruda de cada objeto externo. El sync compara proveedor actual contra el snapshot importado anterior para detectar cambios reales de la nube, no contra la ficha local ya normalizada.
- La version importada conserva valores visibles/originales de Google, Apple, Microsoft u otro proveedor y datos adicionales que aun no viven en la ficha local, como cumpleanos. La ficha local se crea inicialmente como vaciado del snapshot importado, aplicando solo reglas oficiales de normalizacion app. Despues, una funcion separada revisa duplicados, fusiones y diferencias entre snapshots importados y contactos locales.
- Con ese modelo, un contacto local puede quedar vinculado a dos o mas snapshots importados de proveedor si el usuario fusiona duplicados. La importacion no se bloquea por duplicados; al finalizar, la app puede levantar una alerta de calidad en Cuenta si detecta contactos locales o importados pendientes de regularizar. Hasta regularizar duplicados criticos, la app puede bloquear o advertir antes de sincronizar correos/calendarios para evitar asociar interacciones a fichas equivocadas.
- Las interacciones soportan multiples participantes mediante `interaction_participants`.
- `interactions.id` debe ser el ID propio de la app para el evento visible/editable por el usuario. Los IDs de Gmail, Calendar, Apple, Microsoft u otros proveedores deben vivir como referencias externas, idealmente en `external_interaction_sources`, para no mezclar la fuente de verdad de la app con objetos de proveedor.
- El contenido original de una fuente conectada debe conservarse separado de la minuta editable. `source_detail` o una tabla externa equivalente conserva el dato crudo del proveedor; `user_notes_raw` es la version editable por el usuario y la fuente principal para Coach.
...
- `external_interaction_read_diagnostics` guarda la ultima lectura cruda por usuario/proveedor/recurso para objetos externos que no necesariamente pasan a `interactions`, por ejemplo eventos Calendar devueltos por Google pero filtrados por alcance o sin contacto mapeado. Es diagnostico y no reemplaza `external_interaction_sources`.
...
- `sync_run_logs` registra pasos de procesos de sync/importacion para diagnostico operativo; no reemplaza `audit_log`, que registra cambios de negocio.
- Todas las tablas privadas tienen `user_id` y RLS por usuario. Las tablas globales, como `service_connectors`, `plans` y el maestro de empresas headhunter, no se filtran por usuario; deben usar politicas de lectura global y escritura restringida a administradores cuando exista perfilamiento real.
...
- Mantener la app local operativa como referencia hasta cortar el uso.
- Poblar la app cloud desde conectores Google y migraciones separadas solo cuando sea necesario para continuidad.
...
- Formato exacto de migracion futura para minutas, referidos, estados y preferencias que no vengan desde conectores.
...
- 2026-07-20: Se vuelve el modelo agnostico a fuente externa y se agregan cargas/exportaciones controladas.
...
- 2026-07-22: Se define una herramienta transicional de continuidad para desarrollo.
...
- 2026-07-27: Se crearon herramientas transicionales para continuidad de datos durante el desarrollo.
- 2026-08-11: Las herramientas transicionales se movieron a `tools/legacy_migration`; el schema cloud y el runtime no deben depender de campos heredados.
