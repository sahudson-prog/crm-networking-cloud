-- Coffeecito PROD bootstrap: access functions and Auth trigger.
...
alter function public.is_email_authorized_for_app_access(text) owner to postgres;
alter function public.hook_enforce_app_access_allowlist(jsonb) owner to postgres;
alter function public.app_access_status_for_user(uuid) owner to postgres;
alter function public.current_user_app_access_status() owner to postgres;
alter function public.user_has_capability_for_user(uuid, text) owner to postgres;
alter function public.current_user_has_capability(text) owner to postgres;
alter function public.admin_list_app_access_allowlist() owner to postgres;
alter function public.admin_authorize_app_access_email(text, text) owner to postgres;
alter function public.admin_revoke_app_access_email(text) owner to postgres;
alter function public.admin_get_app_access_diagnostic(uuid) owner to postgres;
alter function public.current_user_has_app_access() owner to postgres;
